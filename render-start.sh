...
# PUT/overwrite a small text file (needs SHA if exists)
upsert_text_file() {
  local path="$1"     # e.g. backup/LATEST
  local msg="$2"
  local content="$3"  # plain text content

  # get existing sha (if any)
  sha=$(curl -sS -H "$AUTH_HEADER" "${GH_API}/contents/${path}?ref=${BACKUP_BRANCH}" \
  ...
  if [ -n "$sha" ]; then
    curl -sS -X PUT \
      ...
      -d "{\"message\":\"${msg}\",\"content\":\"${b64}\",\"branch\":\"${BACKUP_BRANCH}\",\"sha\":\"${sha}\"}" \
      "${GH_API}/contents/${path}" >/dev/null
  else
    curl -sS -X PUT \
      -H "$AUTH_HEADER" -H "Content-Type: application/json" \
      -d "{\"message\":\"${msg}\",\"content\":\"${b64}\",\"branch\":\"${BACKUP_BRANCH}\"}" \
      "${GH_API}/contents/${path}" >/dev/null
  fi
}
